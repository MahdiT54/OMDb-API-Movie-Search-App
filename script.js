...
function findMovies() {
  ...
  if (searchTerm.length > 0) {
    ...
    filterToggle.classList.remove("hide-search-list");
    loadMovies(searchTerm);
  } else {
    searchList.classList.add("hide-search-list");
    filterToggle.classList.add("hide-search-list");
  }
}
  ...
  searchListMovies.forEach((movie) => {
    movie.addEventListener("click", async () => {
      ...
      const result = await fetch(
        `https://www.omdbapi.com/?i=${movie.dataset.id}&apikey=1c44ead`
      );
...
function displayMovieDetails(details) {
  resultGrid.innerHTML = `
    ...
    <img src="${
      details.Poster !== "N/A" ? details.Poster : "./assets/image_not_found.png"
    }" alt="movie poster" />
    </div>
